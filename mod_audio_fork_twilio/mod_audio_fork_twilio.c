...
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_audio_fork_twilio_shutdown);
SWITCH_MODULE_RUNTIME_FUNCTION(mod_audio_fork_twilio_runtime);
SWITCH_MODULE_LOAD_FUNCTION(mod_audio_fork_twilio_load);

SWITCH_MODULE_DEFINITION(mod_audio_fork_twilio, mod_audio_fork_twilio_load, mod_audio_fork_twilio_shutdown, NULL /*mod_audio_fork_twilio_runtime*/);
									 ...
									 const char *bugname)
{
	...
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO,
					  "mod_audio_fork_twilio (%s): streaming %d sampling to %s path %s port %d tls: %s.\n",
					  bugname, sampling, host, path, port, sslFlags ? "yes" : "no");

	if (switch_channel_get_private(channel, bugname))
	{
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "mod_audio_fork_twilio: bug %s already attached!\n", bugname);
		return SWITCH_STATUS_FALSE;
	}
	...
	{
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "mod_audio_fork_twilio: channel must have reached pre-answer status before calling start!\n");
		return SWITCH_STATUS_FALSE;
	}
	...
	{
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "Error mod_audio_fork_twilio session cannot connect.\n");
		return SWITCH_STATUS_FALSE;
	}
...
static switch_status_t do_pauseresume(switch_core_session_t *session, const char *bugname, int pause)
{
	...
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "mod_audio_fork_twilio (%s): %s\n", bugname, pause ? "pause" : "resume");
...
static switch_status_t do_graceful_shutdown(switch_core_session_t *session, const char *bugname)
{
	...
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "mod_audio_fork_twilio (%s): do_graceful_shutdown \n", bugname);
...
SWITCH_STANDARD_API(fork_function)
{
	...
	assert(cmd);
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "mod_audio_fork_twilio cmd: %s\n", cmd);
	...
	else
	{
		...
		if ((lsession = switch_core_session_locate(argv[0])))
		{
			...
			{
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "unsupported mod_audio_fork_twilio cmd: %s\n", argv[1]);
			}
...
SWITCH_MODULE_LOAD_FUNCTION(mod_audio_fork_twilio_load)
{
	...
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "mod_audio_fork_twilio API loading..\n");
	...
	*module_interface = switch_loadable_module_create_module_interface(pool, modname);
		...
		switch_event_reserve_subclass(EVENT_DISCONNECT) != SWITCH_STATUS_SUCCESS)
	{
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Couldn't register an event subclass for mod_audio_fork_twilio API.\n");
...
/*
  ...
  Macro expands to: switch_status_t mod_audio_fork_twilio_runtime()
*/
/*
SWITCH_MODULE_RUNTIME_FUNCTION(mod_audio_fork_twilio_runtime)
{
