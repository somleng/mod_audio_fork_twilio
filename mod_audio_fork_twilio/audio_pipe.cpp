                            ...
                            void *user, void *in, size_t len)
{
  ...
  case LWS_CALLBACK_CLIENT_WRITEABLE:
  {
    ...
    if (ap->isGracefulShutdown())
    {
      ...
      int sent = lws_write(wsi, (unsigned char *)ap->m_audio_buffer_out + LWS_PRE, 0, LWS_WRITE_BINARY);
      return 0;
    }
...
AudioPipe::AudioPipe(const char *uuid,
                     ...
                     const char *password,
                     const char *bugname,
                     notifyHandler_t callback) : m_uuid(uuid),
                                                 m_host(host),
                                                 m_port(port),
                                                 m_path(path),
                                                 m_sslFlags(sslFlags),
...
{
  assert(m_audio_buffer_out != nullptr);
...
{
  if (m_audio_buffer_out_write_offset > LWS_PRE)
    addPendingWrite(this);
