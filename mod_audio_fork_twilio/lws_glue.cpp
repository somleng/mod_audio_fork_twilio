...
namespace
{
                                 ...
                                 char *account_sid, char *call_sid, const char *bugname, responseHandler_t responseHandler)
  {
    ...
    TwilioHelper *th = new TwilioHelper(tech_pvt->sessionId, account_sid, call_sid);
    if (!th)
    {
...
extern "C"
{
  ...
  {
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "mod_audio_fork_twilio: fork_init\n");
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "mod_audio_fork_twilio: audio buffer (in secs):    %d secs\n", nAudioBufferSecs);
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "mod_audio_fork_twilio: sub-protocol:              %s\n", mySubProtocolName);
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "mod_audio_fork_twilio: lws service threads:       %d\n", nServiceThreads);
  ...
  {
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "mod_audio_fork_twilio: fork_cleanup\n");
    bool cleanup = false;
