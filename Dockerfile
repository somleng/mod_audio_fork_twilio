...
#COPY ./files/* /tmp/
RUN apt-get update && apt-get install -y --quiet --no-install-recommends git ca-certificates
RUN git clone https://github.com/drachtio/docker-drachtio-freeswitch-base.git  
RUN cp -R docker-drachtio-freeswitch-base/files/* /tmp/ 
RUN ls -la /tmp
...
RUN for i in $(seq 1 8); do mkdir -p "/usr/share/man/man${i}"; done \
    ...
    RUN ./bootstrap.sh -j 
    RUN ./configure --enable-tcmalloc=yes --with-lws=yes --with-extra=yes --with-aws=yes 
    RUN make 
    ...
    COPY mod_audio_fork_twilio /usr/local/src/freeswitch/src/mod/applications/mod_audio_fork_twilio

    WORKDIR /usr/local/src/freeswitch 
    RUN rm modules.conf && ./bootstrap.sh -j 
    RUN ./configure --enable-tcmalloc=yes --with-lws=yes --with-extra=yes --with-aws=yes 
    RUN make 
